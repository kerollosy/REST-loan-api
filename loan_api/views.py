...
def process_payments(request):
    ...
    # SELECT * FROM payment WHERE status == "PENDING" AND due_date <= (today)
    ...
    if due_payments:
        for payment in due_payments:
            ...
            if loan.status == "PENDING":
                return Response({"error": "Loan is not being funded"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"message": "Payments processed successfully"}, status=status.HTTP_200_OK)

    return Response({"error": "There are no pending payments"}, status=status.HTTP_400_BAD_REQUEST)
